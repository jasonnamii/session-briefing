...
description: "세션 브리핑 자동화. 작업 완료시 결정사항·미결·다음액션을 볼트 Agent-Ops/_session-briefs/에 저장 + _latest.md 동기. 세션 종료·작업 완료·10턴↑·'이어서' 시 자동발동. 10턴↑ 도달시 인-컨텍스트 자가요약 + 파일 저장 동시 실행(UP §3 현재성 일원화). 로드는 _latest.md 1회 read. 세션브리핑, session briefing, 이어서, 작업 완료, 세션 저장 맥락에서 사용. NOT: UP수정(→up-manager), 산출물저장."
...
|------|------|
| 작업 완료 | 자동실행 (제안 아님) |
| 10턴↑ 도달 | 인-컨텍스트 자가요약(핵심결정·미결·현위치 1문단) + 파일 저장 동시 실행 + 1줄 보고 |
| 형 "이어서" | `_latest.md` 1회 read → 작업 재개 |
| 새 세션 시작 | `_latest.md` 1회 read → 자동 로드 |
| 형 "생략" | 1회 스킵 |

이유: UP v37.0 §3 현재성 "장기대화(10턴↑): 핵심 결정·미결·현위치 자가요약"과 일원화. 10턴 = 자가요약(인-컨텍스트 1문단) + 파일 저장(다음 세션 대비) 동시. 기존 15턴에서 10턴으로 단축해 컨텍스트 유실 방지.
