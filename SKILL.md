...
description: "세션 브리핑 자동화. 작업 완료시 결정사항·미결·다음액션을 볼트 Agent-Ops/_session-briefs/에 DC write_file로 저장 + _latest.md 동기. 세션 종료·작업 완료·15턴↑·'이어서' 시 자동발동. 로드는 _latest.md 1회 read. 세션브리핑, session briefing, 이어서, 작업 완료, 세션 저장 맥락에서 사용. NOT: UP수정(→up-manager), 산출물저장(→deliverable-engine)."
---
...
PATH   = {VAULT}/Agent-Ops/_session-briefs/YYMMDD_[태그]_작업명.md
LATEST = {VAULT}/Agent-Ops/_session-briefs/_latest.md
WRITE_TOOL = DC write_file (obsidian MCP는 read-only — UP §OBSIDIAN)
READ_TOOL  = DC read_file 또는 obsidian read_note
```

### TIME_ACQUIRE
env date의 YYMMDD 사용. HHmm 파일명에서 제거. 같은 날 재저장 시 기존 파일 덮어쓰기(중복 접미사 불필요).

이유: bash date 호출은 샌드박스 왕복 2-5초. 날짜 단위 식별로 충분. 덮어쓰기로 list_directory 호출 제거.
...
| 저장 = DC write_file 2회 (본파일 + _latest.md) | 로드 시 -2회 읽기로 상쇄 |
...
- **obsidian MCP로 쓰기 시도:** UP §OBSIDIAN read-only HARD. 반드시 DC write_file 사용
